...
\begin{example}{Scanners \& longest possible matches}\\
    ...
    \end{itemize}\\
    \indent It is not the scanner's job to solve syntax errors. $12.0e+ 32$ is invalid syntax, but
    it is not our job (at least the scanner's!) to say that this is incorrect. \\
    \indent Instead, the scanner will try its best to create the longest possible match, tokenising:
